...
int clPeak::runComputeShort(cl::CommandQueue &queue, cl::Program &prog, device_info_t &devInfo)
{
  ...
  try
  {
    log->print(NEWLINE TAB TAB "Short compute (GIOPS)" NEWLINE);
    log->xmlOpenTag("short_compute");
    ...
    cl::Buffer outputBuf = cl::Buffer(ctx, get_write_mem_flags(), (globalWIs * sizeof(cl_short)));
    ...
    if (!forceTest || strcmp(specifiedTestName, "int") == 0)
    {
      ...
      workPerWI = 2048*4; // Indicates short operations executed per work-item
